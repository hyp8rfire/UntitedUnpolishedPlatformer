...
func _process(delta: float) -> void: #godot version of update loop
	...
	if chargeddashcombo > 0 and !Godmode:
		if boost > 1.5 and Score < 2000:
			boost = 1.5
		if boost > 2 and Score < 3000:
			boost = 2
	if closecallbonus == true:
		boost *= 1.25
	if lightingstruck != 0:
		boost *= (1+(lightingstruck*.1))
	if Godmode:
		boost *= (1.0+ chargeddashcombo*.02)
		if Score > 1000:
			boost *= (1.0+ chargeddashcombo*.03)
		if Score > 2000:
			boost *= (1.0+ chargeddashcombo*.05)
	NORMSPEED = 200 * boost
